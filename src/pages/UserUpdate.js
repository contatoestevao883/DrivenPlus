...
export default function UserUpdate() {
    ...
    const { setNome, setCPF, setEmail, nome, CPF, senha, email, setSenha, setNovaSenha, novaSenha, userUpdate} = useContext(AuthContext)
    return(
        <>
            <Link to={`/users/${idUser}`}>
                <Vector3 src={vector3} />
            </Link>
            <DivContainer onSubmit={userUpdate}>
                ...
                <Input type= "password" placeholder="Senha atual" value={senha} onChange={e => setSenha(e.target.value)} required/>
